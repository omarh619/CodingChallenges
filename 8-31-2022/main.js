...
//P: (string), if string includes but or and add apparently after it unless it already has it, no duplicates, if string is blank return ''
//R: return string adding apparently after an and or but
//E:
/* 'It was great and I've never been on live television before but sometimes I don't watch this.'
Output 1
...
function apparently(string) {
    ...
    // loop each word and search for 'and'. add apparently after if not already there
    for(let i=0; i<arr.length; i++){
    if(arr[i]==='and' &&arr[i+1]!=='apparently'){
    arr[i]='and apparently';
    }
    else{
        //same but now looking for 'but
    if(arr[i]==='but'&& arr[i+1]!=='apparently'){
    arr[i]='but apparently';
    }
    }
    }
    //return string joined and turned to string
